 ...
 * 6. Set NEXT_PUBLIC_HUBSPOT_PORTAL_ID and NEXT_PUBLIC_HUBSPOT_FORM_GUID in .env.local!
 * 
 * ----------------------------------------------------------------------------
 ...
 * 4. Copy the Web App URL and paste it into NEXT_PUBLIC_LEAD_COLLECTION_ENDPOINT in .env.local.
 * 
 * ----------------------------------------------------------------------------
 ...
 * 2. Paste your API endpoint URL into NEXT_PUBLIC_LEAD_COLLECTION_ENDPOINT in .env.local.
 * ============================================================================
 */

// Environment Variables (Loaded securely from .env.local via Next.js process.env)
export const HUBSPOT_PORTAL_ID: string = process.env.NEXT_PUBLIC_HUBSPOT_PORTAL_ID || '';
export const HUBSPOT_FORM_GUID: string = process.env.NEXT_PUBLIC_HUBSPOT_FORM_GUID || '';
export const LEAD_COLLECTION_ENDPOINT: string = process.env.NEXT_PUBLIC_LEAD_COLLECTION_ENDPOINT || '';
...
export async function submitLeadData(payload: LeadPayload): Promise<void> {
  ...
  if (!HUBSPOT_PORTAL_ID.trim() && !LEAD_COLLECTION_ENDPOINT.trim()) {
    console.info(
      '[Lead Collection] Endpoint is empty. Lead data logged locally. Configure NEXT_PUBLIC_HUBSPOT_PORTAL_ID or NEXT_PUBLIC_LEAD_COLLECTION_ENDPOINT in .env.local to collect leads directly into HubSpot, Google Sheets, or PostgreSQL.'
    );
  }
}
